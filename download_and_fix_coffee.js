...
(async () => {
    ...
    if (fs.existsSync(scriptPath)) {
        ...
        // Use a simpler approach: define the new images array and replace item by item?
        // Or just hardcode the replacement since we know the order.
        
        // We will do string replacement for the current image paths with the new ones.
        // But the current paths are duplicates ("Cold drinks.jpg", "Brew.jpg").
        // So we need to be careful to replace the CORRECT one for each ID.
        
        // Let's replace the whole products array logic again, it's safer.
        const newProducts = [
            {
                id: 1,
                name: "Velvet Flat White",
