document.addEventListener('DOMContentLoaded', () => {
    ...
    varietyOptions.forEach(option => {
        option.addEventListener('click', () => {
            ...
            currentVarietyPrice = parseFloat(option.dataset.price);
            updateTotal();
        });
    });
    ...
    function updateTotal() {
        const total = (basePrice + currentVarietyPrice + currentExtrasPrice) * currentQuantity;
    ...
    if (addToOrderBtn) {
        addToOrderBtn.addEventListener('click', () => {
            const finalName = decodeURIComponent(n || 'Artisanal Bread');
            const item = {
                id: 'bread-' + Date.now(),
                name: finalName,
                price: (basePrice + currentVarietyPrice + currentExtrasPrice),
            ...
            varietyOptions.forEach(option => {
                if (option.classList.contains('border-primary')) {
                    item.options.push({ name: 'Variety: ' + option.dataset.name, price: parseFloat(option.dataset.price) });
                }
            });
