...
function renderProducts(filter = 'all', query = '') {
    ...
    grid.innerHTML = filteredProducts.map((product, index) => {
        const name = typeof getMenuTranslation === 'function' ? getMenuTranslation(product, 'name') : product.name;
        const desc = typeof getMenuTranslation === 'function' ? getMenuTranslation(product, 'description') : product.description;
        ...
        <div onclick="redirectToCustomization('${product.id}')" class="flex flex-col bg-white dark:bg-slate-800 p-3 rounded-2xl border-2 border-white/70 dark:border-white/10 ring-1 ring-black/5 dark:ring-white/5 ring-offset-1 ring-offset-white dark:ring-offset-[#1a100c] shadow-[0_4px_14px_rgba(0,0,0,0.12)] hover:shadow-[0_10px_24px_rgba(0,0,0,0.18)] fade-in-up cursor-pointer group transition-all duration-300 transform hover:-translate-y-1" style="animation-delay: ${index * 0.05}s">
            <div class="product-image w-full h-44 rounded-xl bg-cover bg-center mb-3 relative overflow-hidden" role="img" aria-label="${name}" style="background-image: url('${product.image}');">
            ...
            <div class="flex-1 flex flex-col px-1">
                ...
                <p class="text-[10px] text-gray-500 dark:text-gray-400 line-clamp-2 mb-3 leading-relaxed">${desc}</p>
...
function redirectToCustomization(productId) {
    ...
    if (product) {
        ...
        } else if (name.includes('latte') || name.includes('lait') || name.includes('macchiato') || name.includes('cortado') || name.includes('caffè')) {
