    ...
    try {
        ...
        if (result.success) {
            ...
            if (statsCards.length >= 2) {
                statsCards[0].textContent = `$${totalSales.toFixed(0)}`;
                  ...
                  <div class="flex items-center gap-6">
                    ...
                    <div>
                      <div class="flex items-center gap-3 mb-1">
                        <span class="text-sm font-bold">Total: $${Number(order.total_amount).toFixed(2)}</span>
...
function renderMenuManagerItems(items) {
    ...
    grid.innerHTML = items.map(item => `
        <div class="bg-white dark:bg-[#2a1e19] p-4 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm">
            ...
            <div class="flex items-center justify-between">
                <span class="text-primary font-black">$${Number(item.price).toFixed(2)}</span>
        ...
        cartList.innerHTML = posCart.map(item => `
            <div class="flex items-center gap-4 bg-gray-50 dark:bg-black/20 p-3 rounded-2xl border border-gray-100 dark:border-white/5">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-[10px] text-gray-400 font-bold tracking-widest uppercase">$${(item.price * item.quantity).toFixed(2)}</p>
                </div>
